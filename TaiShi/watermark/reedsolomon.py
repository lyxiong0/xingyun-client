# function: reedsolomon erasure codes
# author： heany
# time：2019.5.27
...
try:
    ...
    def bytearray(obj = 0, encoding = "latin-1"): 
        # always use Latin-1 and not UTF8 
        # because Latin-1 maps the first 256 characters to their bytevalue equivalents. 
        # UTF8 may mangle your data (particularly at vale 128)
...
def gf_poly_neg(poly):
    ...
    In GF(2^p), negation does not change the coefficient, so we return the polynomial as-is.'''
    return poly
...
def gf_poly_square(poly):
    ...
    For details, see paper: "A fast software implementation for arithmetic operations in GF (2n)". De Win, E., Bosselaers, A., Vandenberghe, S., De Gersem, P., & Vandewalle, J. (1996, January). In Advances in Cryptology - Asiacrypt'96 (pp. 65-76). Springer Berlin Heidelberg.'''
    length = len(poly)
...
class RSCodec(object):
    ...
    def decode(self, data, erase_pos=None, only_erasures=False):
        ...
        # erase_pos is a list of positions where you know (or greatly suspect at least) 
        # there is an erasure (ie, wrong character but you know it's at this position). 
        # Just input the list of all positions you know there are errors, 
        # and this method will automatically split the erasures positions to attach to the corresponding data chunk.
        if isinstance(data, str):
            data = bytearray(data, "latin-1")
        dec = bytearray()
...
# rs = RSCodec(170)
# message = 'hello world Please note that the first paragraph of a section or subsection is\
# not indented. The first paragraph that follows a table, figure,\
# equation etc. does not need an indent, either.'
# print(len(message))
# enc = rs.encode(message)
# # print(enc)
# # print([ch for ch in enc])
# print('length: ', len(enc))

# # =============== make error=============
# enc[:10] = [i*10 for i in range(10,20)]
